...
describe('Torch Token Tests >', () => {
  ...
  describe('Token tests for D&D5e actors, scene, and tokens >', () => {
    ...
    it('Light source selection', async () => {
      // Set up data
      let actor = new MockActor("1234567", "Versatile", [
        new MockItem("Torch", 1), 
        new MockItem("HoodedLantern") 
      ], 15, 30);
      globalThis.game = new MockGame(
        "dnd5e", [actor], false,
        {
    ...
    it('Cycle of token states - torch', async () => {
      ...
      let library = await SourceLibrary.load("dnd5e", 10, 20);
      // Perform test
      let token = new TorchToken(new MockToken(actor, "HoodedLantern"), library);
      ...
      assert.equal(token.lightSourceState, token.STATE_OFF, "Token is initially off");
      await token.forceStateOff();
      assert.equal(token.lightSourceState, token.STATE_OFF, "Token is off when forced to be off when it already is");
      exhausted = token.lightSourceIsExhausted("Torch");
      assert.equal(exhausted, false, "Torches are not exhausted after initially forcing off from off");
    ...
    it('Cycle of token states - hooded lantern', async () => {
      ...
      assert.equal(token.lightSourceState, token.STATE_OFF, "Hooded lantern is initially off");
      await token.advanceState();
      assert.equal(token.lightSourceState, token.STATE_ON, "Token is on after first advancement");
      await token.advanceState();
      assert.equal(token.lightSourceState, token.STATE_DIM, "Token is dim after second advancement");
      await token.advanceState();
      assert.equal(token.lightSourceState, token.STATE_OFF, "Token is off after third advancement");
